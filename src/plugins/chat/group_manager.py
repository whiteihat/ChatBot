...
class GroupManager:
    ...
    def __init__(self, config_dir: str = None):
        ...
        if not config_dir:
            self.config_dir = (
                Path(os.path.dirname(os.path.abspath(__file__))) / "group_configs"
            )
        else:
        ...
        if group_id in self.groups:
            ...
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    json.dump(
                        self.groups[group_id].to_dict(), f, ensure_ascii=False, indent=4
                    )
...
ResourceManager.add_to_registry(
    name="group_manager", initializer=initialize_group_manager
)
